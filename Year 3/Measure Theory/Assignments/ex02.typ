...
#proof[
  בשביל שפונקציה $f: (RR, cal(A)) arrow.r (RR, cal(B)(RR))$ תהיה מדידה, צריך להתקיים לכל $B subset.eq RR$ ש־$f^(-1)(B) in cal(A)$, כלומר לכל $y in RR$ צריך להתקיים $f^(-1)({y}) in cal(A)$.\
  נשים לב שלא ייתכן שעבור $y_1 != y_2$ נקבל ש־$f^(-1)({y_1}), f^(-1)({y_2})$ שניהם לא בני־מנייה, כי המשלים של שניהם יהיה בן־מנייה ואם יש לנו שתי קבוצות זרות לא בנות־מנייה זה גורר ש־$RR$ הוא איחוד של שתי קבוצות בנות־מנייה אבל $RR$ איננו בן־מנייה. אז יש $c in RR$ יחיד כך ש־$abs(f^(-1)({c})^c)<=aleph_0$.\
  כמו־כן, $union.big_(y in RR) f^(-1)({y})=RR$, כי אם כל $f^(-1)({y})$ היה בן־מנייה, אז $RR$ היה איחוד בן־מנייה של קבוצות בנות־מנייה ולכן בן־מנייה וזאת כמובן סתירה. אז יש לפחות $c in RR$ אחד כך ש־$f^(-1)({c})$ לא בת־מנייה, כלומר המשלים שלו הוא בן־מנייה,  אז לכל $y!=c, f^(-1)({c})$ הוא בן־מנייה.\
  אבל זה בידיוק אומר ש־$f$ היא מדידה אם ורק אם יש $c in RR$ ו־$N subset RR$ כך ש־$N$ בת־מנייה ו־$f(x)=c$ לכל $x in RR without N$.\
  אז כל $B$ מקיימת ש־$f^(-1)(B) subset N$ ולכן בת־מנייה או $f^(-1)(B) subset RR without N$ כלומר לא בת־מנייה (אם הסרנו קבוצה בת־מנייה מקבוצה לא בת־מנייה, היא כמובן לא בת־מנייה), כלומר, $f^(-1) in cal(A)$.\
  עם $mu$ מהסעיף הקודם מתקיים $mu(RR without N) = 1, mu(N) = 0$, ואם נחלק את האינטגרל לחלוקה בהתאם
  $
    integral_RR f d mu = integral_(RR without N) f d mu + integral_N f d mu = c dot.op mu(RR without N) 0 = c ==> integral_RR f d mu = c
  $
]
