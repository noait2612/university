...
#definition[תבנית ביליניארית סימטרית][
  יהי $FF$ שדה כך ש־ $char(FF) != 2$ (כלומר $1_FF + 1_FF != 0_FF$), ויהי $V$ מרחב וקטורי מעל $FF$.
  פונקציה $g: V times V -> FF$ נקראת תבנית בילינארית סימטרית אם היא מקיימת את האקסיומות הבאות
...
#proof[ נראה את הראשון והשני נובע באופן דומה
  $
...
#example[התבנית הביליניארית הסימטרית הטריוויאלית][
...
#definition[מטריצת גראם][
  יהי $V$ מרחב וקטורי ממימד סופי מעל $FF$, $g$ תבנית בילינארית סימטרית, ו־ $cal(B) = (v_1, dots.h, v_n)$ בסיס של $V$.
...
#proposition[
  יהי $(V, chevron.l dot, dot chevron.r)$ מרחב מכפלה פנימית ממימד סופי מעל $bb(R)$, $g(v, w) = chevron.l v, w chevron.r$, ו־ $cal(B) = (v_1, dots.h, v_n)$ בסיס אורתונורמלי של $V$. אזי $[g]_cal(B) = I_n$.
]
...
#proof[
  ...
  $
    g(v, w) = g(sum_(i=1)^n c_i v_i, sum_(j=1)^n d_j v_j) = sum_(i=1)^n sum_(j=1)^n c_i d_j g(v_i, v_j) = sum_(i=1)^n sum_(j=1)^n c_i d_j g_(i,j)
  $
  ...
  $
    [v]_cal(B)^t [g]_cal(B) [w]_cal(B) = mat(c_1, dots.h, c_n) mat(
      ...
      dots.v, dots.down, dots.v;
      g_(n,1), dots.h, g_(n,n)
    ) mat(d_1; dots.v; d_n) = sum_(i=1)^n sum_(j=1)^n c_i d_j g_(i,j)
  $
...
#proposition[
  יהי $V$ מרחב וקטורי ממימד סופי מעל $FF$, $g$ תבנית בילינארית סימטרית, ו־ $cal(B), cal(B)'$ בסיסים של $V$.
...
#proposition[
  יהי $V$ מרחב וקטורי ממימד סופי מעל $FF$, $g$ תבנית בילינארית סימטרית, ו־$cal(B)$ בסיס של $V$ ותהי $A in M_n (FF)$ חופפת ל־ $[g]_cal(B)$.\
...
נראה כי לא רק ששתי מטריצות המייצגות את אותה תבנית ביליניארית סימטרית g בבסיסים שונים הן חופפות, אלא שגם ההפך הוא נכון.

#proposition[
  יהיו $V$ מרחב וקטורי נוצר סופית מעל $FF$ ממימד $n$, $B$ בסיס של $V$, $g$ תבנית ביליניארית סימטרית על $V$ ו־$C in M_n (FF)$ מטריצה שחופפת $[g]_B$.\
  אזי קיים בסיס $B^prime$ של $V$ עבורו $C=[g]_(B^prime)$.
]

#proof[
  קיימת $P in M_n (FF)$ הפיכה כך ש־$C=P^t [g]_B P$ ומאחר ש־$P$ הפיכה קיים בסיס $B^prime$ כך ש־$P$ היא מטריצת מעבר בסיסים בין $B$ ל־$B^prime$ כלומר $P=[Id_v]_B^(B^prime)$ ולכן
...
#definition[תבנית ריבועית][
  יהי $V$ מרחב וקטורי מעל $FF$ עם מציין שונה מ־$2$ ו־$g$ תבנית ביליניארית סימטרית על $V$. תבנית ריבועית שמתאימה ל־$g$ היא $g:V arrow FF$ המוגדרת על־ידי
  $ q(v) = g(v,v) $
]
...
#proposition[נוסחת הפולריזציה][
  יהי $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית על $V$ ו־$q$ תבנית ריבועית המתאימה ל־$g$. אזי לכל $v,w in V$ מתקיים
...
#proof[
  $
    ...
    =_((1)) cancel(g(v,v), stroke: #(paint: teal)) + g(v,w) + g(w,v) + cancel(g(w,w), stroke: #(paint: purple)) - cancel(g(v,v), stroke: #(paint: teal)) - cancel(g(w,w), stroke: #(paint: purple))
    =_((2)) 2g(v,w)
  $
...
#corollary[
  אם תבנית ביליניארית סימטרית היא לא תבנית ה־$0$ אז גם התבנית הריבועית המתאימה לה היא לא תבנית ה־$0$.\
  כלומר, אם $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית ששונה מתבנית ה־$0$ אזי קיים $v in V$ כך ש־$q(v)!=0$.
]

#definition[ניצבות][
  יהי $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית על $V$ ו־$v,w in V$. אומרים כי $v$ ו־$w$ ניצבים ומסמנים $v perp w$ כאשר $g(v,w)=0$.
]

#definition[תת־מרחב ניצב][
  יהי $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית על $V$ ו־$S subset.eq V$. נגדיר את תת־המרחב הניצב $S^perp$
...
#proposition[
  יהי $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית על $V$ ויהי $v in V$ כך ש־$g(v,v)!=0$, אזי
  + $dim{v}^perp = n-1$
...
#definition[בסיס אורתוגונלי][
  יהי $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית על $V$ ו־$B={v_1, dots.h, v_n }$ בסיס של $V$. $B$ נקרא אורתוגונלי כאשר $v_i perp v_j$ לכל $1<=i<j<=n$.
]

#proposition[
  יהי $V$ מרחב וקטורי מעל $FF$ ו־$g$ תבנית ביליניארית סימטרית על $V$. אז ב־$V$ קיים בסיס אורתוגונלי.
]
...
#proof[
  ...
  אם לכל $v in V$ מתקיים ש־$g(v,v)=0$ אזי $g(v,w)=0$ לכל $v,w in V$ מנוסחת הפולריזציה ובפרט נקבל כי כל בסיס של $V$ הוא אורתוגונלי.\
  אחרת, קיים $v in V$ כך ש־$g(v,v)!=0$ ולפי הטענה הקודמת מתקיים שהמימד של ${v}^perp$ הוא $n-1$ ומהנחת האינדוקציה ל־${v}^perp$ קיים בסיס אורתוגונלי \
  $B^prime = {v_1, dots.h, v_(n-1)}$ ומהטענה הקודמת ${v}^perp plus.o {v} = V$ וגם $v perp v_i$ לכל $1<=i<=n-1$ ולכן $B={v_1, dots.h, v_(n-1), v}$ הוא בסיס אורתוגונלי.
]

#proposition[
  יהי $V$ מרחב וקטורי מעל $FF$, $g$ תבנית ביליניארית סימטרית על $V$ ו־$B={v_1, dots.h}$ בסיס של $V$. אזי $B$ אורתוגונלי אם ורק אם מטריצת גראם $[g]_B$ אלכסונית.
]

#proof[
  $<==$ נניח ש־$B$ בסיס אורתוגונלי ולכן לכל $i!=j$ מתקיים $v_i perp v_j$ כלומר $g(v_i, v_j) = 0$ ולכן כל הכניסות מחוץ לאלכסון של מטריצת גראם $[g]_B$ הן אפסים ולכן זאת מטריצת גראם אלכסונית.\
  $==>$ נניח שמטריצת גראם $[g]_B$ אלכסונית ולכל $i!=j$ מתקיים $g(v_i, v_j) = g_(i,j) = 0$ ולכן $v_i perp v_j$, כלומר $B$ בסיס אורתוגונלי.
]

#corollary[
  יהי $V$ מרחב וקטורי מעל $FF$ ו־$g$ תבנית ביליניארית סימטרית על $V$. אז קיים בסיס $B$ של $V$ כך ש־$[g]_B$ אלכסונית.
]
...
#definition[הומומורפיזם הקנוני][
  ...
  נסמן ב־$pi: FF^times arrow frac(FF^times, (FF^times)^2, style: "horizontal")$ ההומומורפיזם הקאנוני כלומר $pi(a)=a(FF^times)^2$ לכל $a in FF^times$.\
]

#definition[קבוצת נציגים][
  תת־קבוצה $R subset.eq FF^times$ תיקרא קבוצת נציגים כאשר $pi|_R : R arrow frac(FF^times, (FF^times)^2, style: "horizontal")$ הוא חד־חד ערכי ועל.\
]
...
#example[
  + $FF=CC$ אז $(CC^times)^2 = CC^times$ ומתקיים $abs(frac(CC^times, (CC^times)^2, style: "horizontal"))=1$ וניתן לבחור $R={1}$
  + $FF=RR$ אז $(CC^times)^2 = {x in RR^times bar x>0}$ ומתקיים $abs(frac(RR^times, (RR^times)^2, style: "horizontal"))=2$ וניתן לבחור $R={-1,1}$
]

#proposition[
  יהי $p$ ראשוני ו־$p!=2$ אז $abs(frac(FF_p^times, (FF_p^times)^2, style: "horizontal"))=2$. יתר על־כן, לכל $r in FF_p^times without (FF_p^times)^2$ ניתן לבחור קבוצת נציגים להיות $R={1,r}$.
]
...
#proof[
  ...
  מתקיים $im(s) tilde.equiv frac(FF_p^times, ker s, style: "horizontal")$ ובנוסף $x in ker s <==> s(x)=1 <==> x^2=1 <==> (x-1)(x+1)=0 <==> x in {-1, 1}$.\
  לכן $ker s = {1, -1}$ ומכאן $abs(ker s)=2$.
  לסיכום
  $
    abs(frac(FF_p^times, (FF_p^times)^2, style: "horizontal"))=abs(frac(FF_p^times, im(s), style: "horizontal")) = abs(FF_p^times) : abs(im(s)) = abs(FF_p^times) : (abs(FF_p^times) : abs(ker s)) = abs(ker s)=2
  $
  היתר על־כן נובע מהגדרת נציגים – לכל אחד מהקוסטים יש נציג יחיד מתוכה ולכן $1$ הוא נציג של הריבועים ו־$r$ נציג של הלא ריבועיים.
]
...
#definition[פעולות עמודה אלמנטריות][
  תהיי $epsilon$ פעולת שורה אלמנטרית על מטריצות מ־$M_n (FF)$ ונסמן ב־$overline(epsilon)$ את פעולת עמודה אלמנטרית המתאימה, כלומר
...
#proposition[
  תהיי $g$ תבנית ביליניארית סימטרית על $FF^n$ ונסמן את $[t]_cal(E)=A$ (כאשר $cal(E)$ הוא הבסיס הסטנדרטי).\
  ...
  נסמן ב־$b_1, dots.h, b_n$ את העמודות של $P$ אז $cal(B) = (b_1, dots.h, b_n)$ הוא בסיס של $FF^n$ וגם $[g]_cal(B)=D$.
]
...
#notation[
  בהינתן תבנית ריבועית $q:V arrow FF$ על מרחב וקטורי $V$ נסמן ב־$g_q : V times V arrow FF$ את התבנית הביליניארית הסימטרית המקיימת $q(v)=g_q (v,v)$.
]
...
#example[
  ...
  הצבה פשוטה מראה שמתקיים $g_q (vec(x_1, x_2, x_3), vec(x_1, x_2, x_3)) = q(vec(x_1, x_2, x_3))$ ועם (מטריצת גראם) מתקיים גם־כן
...
#definition[מרחב ריבועי][
  יהי $V$ מרחב וקטורי נוצר סופית מעל השדה $FF$, $q:V arrow FF$ תבנית ריבועית. הזוג $(V,q)$ נקרא מרחב ריבועי ונגיד כי $(V,q)$ מרחב ריבועי מעל $FF$.\
  בהינתן $(V,q)$ מרחב ריבועי, נסמן ב־$g_q$ את התבנית הביליניארית הסימטרית מעל $FF$ המקיימת $g_q (v,v)=q(v)$.
]
...
]<isomorphic_spaces_iff_gram_matrix_congurte>
...
#claim[
  יהי $(V,q)$ מרחב ריבועי מעל $FF$, $U_1, U_2$ תתי־מרחבים של $V$ כך שהם סכום ישר ניצב עם $cal(B)_1$ ו־$cal(B)_2$ בסיסים של $U_1$ ו־$U_2$ בהתאמה.\
  נסמן $cal(B) = cal(B)_1 union cal(B)_2$ אז $cal(B)$ בסיס של $V$ וגם
...
#proof[
  ...
  אז העמודה הראשונה של $[g_q]_(cal(B)^prime)$ היא עמודת אפסים ולכן היא אינה הפיכה ומחפיפה עם $[g_q]_cal(B)$ נקבל את הנדרש.\
  $==>$ נניח כי $[g_q]_cal(B)$ לא הפיכה ולכן קיים $(0, dots.h, 0) != (c_1, dots.h, c_n) in FF^n$ כך ש־$[g_q]_cal(B) vec(c_1, dots.v, c_n) = vec(0, dots.v, 0)$.\
  נסמן $cal(B) = {v_1, dots.h, v_n}$ ונגדיר $w = c_1 v_1 + dots.h.c + c_n v_n$ ואז $[w]_cal(B) = vec(c_1, dots.v, c_n)$ ומכאן לכל $v in V$ מתקיים
  $
    g_q (v, w) = ([v]_cal(B))^t [g_q]_cal(B) [w]_cal(B) = ([v]_cal(B))^t [g_q]_cal(B) vec(c_1, dots.v, c_n) = ([v]_cal(B))^t vec(0, dots.v, 0) = vec(0, dots.v, 0)
  $
  כלומר $w perp v$ ומכאן $w in V^perp$ ולכן המרחב מנוון.
]

#corollary[
  יהיו $(V,q)$ מרחב ריבועי מעל $FF$, $cal(B) = {v_1, dots.h, v_n}$ בסיס אורתוגונלי של $V$. אזי $(V,q)$ מנוון אם ורק אם קיים $1<=i<=n$ כך ש־$q(v_i) =0$.
]
...
#corollary[
  יהיו $(V,q)$ מרחב ריבועי לא מנוון מעל $FF$ ו־$R subset.eq FF$ קבוצת נציגים מודולו ריבועיים. אזי קיים בסיס $cal(B)$ של $V$ כך ש־$[g_q]_cal(B)$ אלכסונית ואיברי אלכסונה שייכים ל־$R$.
]
...
#proposition[
  יהיו $(V,q)$ מרחב ריבועי מעל $FF$, $W, V$ תתי־מרחבים של $V$ כך ש־$V = U plus.o^perp W$, $(U, q|_U)$ לא מנוון ו־$q|_W (w) =0$ לכל $w in W$. אזי $V^perp = W$.
]

#proof[
  לכל $w in W$, נתון שלכל $v in V$ קיים $u^prime in U$ ו־$w^prime in W$ כך ש־$v=u^prime + w^prime$ ולכן
  $
    g_q (v,w) = underbrace(g_q (u^prime, w), U plus.o^perp W ==> = 0) + underbrace(g_q (w^prime, w), q|_W (w) = 0) = 0
  $
  ולכן $w in V^perp$.\
  כעת יהי $v in V^perp$ וקיימים $u in U$ ו־$w in W$ כך ש־$v=u+w$. נניח בשלילה כי $u!=0$ אז מהיות $(U, q|_U)$ לא מנוון קיים $u^prime in U$ עבורו $g_q (u^prime, u) !=0$ ולכן
  $
...
#proof[
  ...
  מאחר ש־$z in W$ נובע שקיים $0_(RR^n) != v in V$ כך ש־$f(v)=z$ ולכן קיימים $d_(k+1), dots.h, d_n in RR$ לא כולם אפס ומתקיים $v = (0, dots, 0, d_(k+1), dots, d_n)^t$ ולכן
...
#proof[
  ...
  $
  נגדיר $f:(FF_p^times)^2 union {0} arrow A_1$ על־ידי $f(x_1) = a_1 x$ ולכן $f$ היא חד־חד ערכית ועל (ההופכית זה חילוק) ולכן $abs((FF_p^times)^2 union {0}) = abs(A_1) = (p+1)/2$ ובאופן דומה גם $abs(A_2) = (p+1)/2$.\
...
#proof[
  ...
  לכן $(U, q|_U)$ מרחב לא מנוון ומתקיים $FF_p^times subset.eq q|_U (U without {0_U}) subset.eq q(V without {0})$.
]
...
]<classification_theorem_for_finite_fields>

#proof[
  באינדוקציה על $n$: עבור $n=1$ ברור כי $[g_q]_cal(B) = mat(1)$ או $[g_q]_cal(B) = mat(r)$ ונניח כי הטענה נכונה למרחב ריבועי ממימד $n-1$.\
...
#example[
  ...
  מהמשפט קיים בסיס $cal(B)$ של $FF_7^2$ כך שמתקיים $[g_q]_cal(B) = mat(1, 0; 0, 1)$.
]
...
#definition[$bold(hat(g)_q comma g_q^v)$][
...
#proof[
  $<==$ נניח שהוא לא ארכימדי אז יהיו $a,b in FF$ ובלי הגבלת הכלליות נניח $norm(a)>=norm(b)$. מתקיים
  $
  ...
  מכאן ש־$norm(a+b) <= root(n, n+1) norm(a)$ ומאחר ש־$lim_(n arrow.r infinity) root(n, n+1)=1$, נסיק כי $norm(a+b)<=norm(a) = max(norm(a), norm(b))$.\
...
#proof[
  $subset.eq$: נובע מההגדרה של $ZZ_((p))$ ש־$ZZ_((p))$ קבוצה סגורה (כי זה כדור סגור) ו־$ZZ subset.eq ZZ_((p))$ כי כל מספר שלם אפשר לכתוב כמספר רציונלי עם מכנה $1$ ולפיכך $closure(ZZ) subset.eq ZZ_((p))$.\
  $supset.eq$: יהי $x in ZZ_((p))$ כלומר $x = m/n in QQ$ כך ש־$m in ZZ, n in NN$ ו־$p divides.not n$.\
  יהי $epsilon > 0$ ונוכיח כי קיים $z in ZZ$ כך ש־$abs(z-x)_(p, alpha) < epsilon$.\
  מכך ש־$0<alpha<1$ קיים $N in NN$ כך ש־$alpha^N < epsilon$ (לדוגמה $N=floor(log_alpha (epsilon))+1$) ומאחר ש־$gcd(n, p^N) =1$ נובע שקיימים $s, t in ZZ$ כך ש־$s p^N + t n = 1$ (מאלגוריתם אוקלידס).\
  מכאן $m s p^N = m - m t n$ אבל $p^N divides m s p^N$ אז אם נגדיר $z = m t$ נקבל את הנדרש, שכן
  $ N<=v_p (m- m t n) = v_p (m-z n)= v_p (m-z n) - underbrace(v_p (n), =0)= tilde(v)_p ((m-z n)/n) = tilde(v)_p (x-z) $
  ומכאן
...
#proof[
  ...
  נרצה להראות ש־$pi$ הוא הומומורפיזם של חוגים ו־$ker pi = p ZZ_((p))$ ולפיכך $ZZ_p slash p ZZ_((p)) tilde.equiv ZZ slash p ZZ_p = FF_p$ ממשפטי האיזומורפיזם.\
  + על – יהי $overline(a) in ZZ slash p ZZ$. מאחר ש־$a in ZZ subset.eq ZZ_((p))$ ו־$a-a=0 in p ZZ_((p))$ נקבל $pi(a)=a+p ZZ = overline(a)$ כלומר על.
  + הומומורפיזם של חוגים – יהיו $x,y in ZZ_((p))$ ויהיו $z_x, z_y in ZZ$ כך שמתקיים $x-z_x, y-z_y in p ZZ_((p))$.\
    מאחר ש־$p ZZ_((p))$ אידיאל של $ZZ_((p))$ נובעים
    $
      (x+y)-(z_x + z_y) = (x-z_x) + (y-z_y) in p ZZ_((p)) \
      x y - z_x z_y = x(y-z_y) + z_y (x-z_x) in p ZZ_((p))
    $
    ולכן $z_x + z_y$ ו־$z_x z_y$ נציגים של $x+y$ ו־$x y$ בהתאמה ומתקיים
    $ pi(x+y) = pi(x)+pi(y), quad pi(x y) = pi(x)pi(y) $
    לבסוף, $1-1=0 in p ZZ_((p))$ ולכן $pi(1)=1 + p ZZ$ ומכאן ש־$pi$ הוא הומומורפיזם של חוגים.
  + $x in ker(pi)$ אם ורק אם $pi(x)=0$ וזה קורה אם ורק אם $pi(x)=0 + p ZZ$ אבל $z=0$ מתאים ל־$x$ אם ורק אם $x-0 in p ZZ_((p))$ כלומר $x in p ZZ_((p))$.
]
...
#proof[
  מאחר ש־$norm(dot.op)$ לא ארכימדית ולא טריוויאלית נובע שלכל $n in NN$ מתקיים $norm(n)<=1$ וקיים $n in NN$ עבורו $norm(n)<1$. \
  נגדיר $p colon.eq min{n in NN bar norm(n)<1}$ ונראה ש־$p$ ראשוני (ברור שהוא לא $1$). נניח בשלילה כי $p$ פריק ולכן קיימים $a,b in NN$ כך ש־$a,b<p$ ו־$p = a b$.\
  ...
  מתקיים ש־$norm(r)=1$ וכמו כן $norm(q)<=1$ ולכן $norm(q p) = norm(q) norm(p) < 1$ ומאחר ש־$norm(p q) != norm(r)$ מתקיים כי $norm(u)=norm(q p + r) = max(norm(q p), norm(r)) = norm(r) = 1$.\
  יהי $v in ZZ$ עם $v<0$ וגם $p divides.not v$ אז $-v in NN$ ו־$p divides.not -v$ ולכן $norm(v)=norm(-v)=1$ ממה שראינו לעיל.\
  נגדיר $alpha=norm(p)$ ויהי $0 != q in QQ$ אז קיימים $0 != m in ZZ, n in NN$ כך ש־$q=m/n$.\
  קיימים $s in ZZ, t in NN$ כך ש־$p divides.not s, p divides.not t$ ו־$m=p^(v_p (m)) s, n = p^(v_p (n)) t$ ואז
  $
...
#proof[
  נניח כי $S$ פתוחה ב־$(QQ, abs(dot.op)_(p, beta))$. אז לכל $x in S$ קיים $r>0$ כך שלכל $y in QQ$, $abs(y-x)_(p, beta) < r$ גורר ש־$y in S$.\
...
#claim[קיום השלמה למרחב מטרי][יהי
  ...
  נסמן ב־$C S(M)$ את הקבוצה של כל סדרות קושי ב־$M$. נגדיר יחס $~$ על $C S(M)$ באופן הבא: $(x_n) ~ (y_n)$ אם ורק אם $lim_(n arrow.r infinity) d(x_n, y_n) = 0$.\
...
#proof[
  + מאחר ש־$O_norm(dot.op) subset.eq hat(O)_norm(dot.op)$ וגם $hat(O)_norm(dot.op)$ סגור מתקיים ש־$closure(O)_norm(dot.op) subset.eq hat(O)_norm(dot.op)$. בכיוון השני, יהי $0!=a in hat(O)_norm(dot.op)$. אז קיימת סדרת קושי $(a_n)_(n=1)^infinity subset.eq FF$ כך ש־$lim_(n arrow.r infinity) a_n = a$
    אז $lim_(n arrow.r infinity) norm(a_n) = norm(a)$. לפי הלמה, קיים $N in NN$ כך שלכל $N<n$ מתקיים ש־$norm(a_n) = norm(a_N) = norm(a)<=1$ ומכאן $a_n in O_norm(dot.op)$. ולכן $a in closure(O)_norm(dot.op)$ כגבול של סדרה.\
  + זהה.
  + נגדיר $phi: closure(FF) arrow closure(hat(FF))$ על־ידי $phi(a+M_norm(dot.op)) = a+hat(M)_norm(dot.op)$ לכל $a in O_norm(dot.op)$ ו־$phi$ מוגדרת היטב ואיזומורפיזם: מוגדרת היטב מאחר ש־$M_norm(dot.op) subset.eq hat(M)_norm(dot.op)$ (ולכן נציגים שקולים נשארים שקולים), והיא הומומורפיזם משום שהפעולות בחוגי מנה מוגדרות ישירות על נציגי המחלקות.\
    מאחר ש־$closure(FF)$ שדה אז $ker phi = {0}$ ולכן $phi$ חד־חד ערכית.\
    בשביל העל, יהי $a in hat(O)_norm(dot.op)$ ולכן קיימת סדרת קושי $(a_n)_(n=1)^infinity subset.eq O_norm(dot.op)$ כך ש־$lim_(n arrow.r infinity) a_n = a$. \
  ...
  4. ברור כי ${norm(a) bar a in FF} subset.eq {norm(a) bar a in hat(FF)}$. בכיוון השני, יהי $0!=a in hat(FF)$. אז קיימת סדרה $(a_n)$ של איברים מ־$FF$ כך ש־$lim_(n arrow.r infinity) a_n$. מהלמה קיים $N in NN$ כך שלכל $N<=n$ מתקיים $norm(a_n) = norm(a_N)$ ומאחר ש־$lim_(n arrow.r infinity) norm(a_n) = norm(a)$ לכל $N<=n$ מתקיים $norm(a_n) = norm(a)$ ולכן $norm(a) = norm(a_n) in {norm(x) bar x in FF}$.
]

== שדה המספרים ה־$p$־אדיים
#notation[שדה המספרים ה־$bold(p)$־אדיים][
...
#proposition[
  תהיי $(a_n)_(n=1)^infinity$ סדרה ב־$ZZ$ כך ש־$0<= a_n <= p-1$ עבור $n in NN_0$ אז גם ב־$QQ_p$ קיים הגבול $sum_(n=0)^infinity a_n p^n = a$ וגם $a in ZZ_p$ ויתר על־כן, $a in p ZZ_p <==> a_0 = 0$.
]
...
#proposition[קיום ויחידות הקירוב ה־$bold(p)$־אדי][
  לכל $x in ZZ_p$ ולכל $k in NN_0$ קיים $r in ZZ$ יחיד כך ש־$0<=r<=p^k-1$ כך ש־$abs(x-r)_p <= 1/p^k$.
]<p_addic_existence_and_unique_approximation>
...
#proof[
  ...
  נשאר להראות יחידות – יהיו $0<=r_1, r_2 <= p^k-1$ כך ש־$abs(x-r_1)_p, abs(x-r_2)_p <= 1/p^k$ מכאן
  $ abs(r_2 - r_1)_p = abs((x-r_1) - (x-r_2))_p <= max{abs(x-r_1)_p, abs(x-r_2)_p} <= 1/p^k $
  ולכן $p^k divides (r_2 - r_1)$ אבל $r_1, r_2 < p^k$ ולכן $r_2 - r_1 = 0$ כלומר $r_1 = r_2$.
]
...
]<corollary_for_p_addic_existence_and_unique_approximation>
...
#proposition[קיום ההצגה ה־$bold(p)$־אדית][
...
]<existence_of_p_addic_presentention>

#proof[
  נגדיר $ a_0 = r_1, quad forall i in NN, space a_i = frac(r_(i+1) - r_(i), p^i) $
  מ@corollary_for_p_addic_existence_and_unique_approximation, $a_i in ZZ$ ומאחר ש־$0<=r_(i+1)<=p^(i+1)-1$ ו־$0<=r_i <= p^i - 1$ מתקיים $-(p^i-1)<= r_(i+1) - r_i <= p^(i+1) - 1$.\
  מאחר ש־$p^i divides r_(i+1) - r_i$ מתקיים $0<=r_(i+1)-r_i <= p^(i+1)-p^i$ ולכן $0<=a_i <= p-1$ ולסיום
...
#proposition[הצגת לורן ה־$bold(p)$־אדית][
...
]<laurent_p_adic_repersention>

#proof[
  קיים $s in ZZ$ כך ש־$abs(x)_p = frac(1, p^s)$ אז נגדיר $x^prime = frac(x, p^s)$ אז $abs(x^prime)_p = frac(abs(x)_p, abs(1/p^s)_p) = 1$ ולכן מ@existence_of_p_addic_presentention קיימת סדרה $(a^prime_n)_(n=0)^infinity subset.eq ZZ$ כך ש־$0<=a^prime_n <= p-1$ וגם \
  $sum_(n=0)^infinity a^prime_n p^n=x^prime$ ומאחר ש־$abs(x^prime)=1$ מתקיים $a^prime_0 != 0$ ולכן נגדיר $a_n = a^prime_(n-s)$ לכל $n in NN$ ו־$s<=n$ ונקבל
  $
    sum_(n=s)^infinity a_n p^n = sum_(n=0)^infinity a_(n+s) p^(n+s) = sum_(n=0)^infinity a^prime_n p^n p^s = x^prime p^s = x
  $
]
...
#proof[
  $subset.eq$ ברור כי $NN_0 subset.eq ZZ_p$ (השדה לא ארכימדי) ו־$ZZ_p$ סגורה ולכן $closure(NN_0) subset.eq closure(ZZ_p) = ZZ_p$.\
  $supset.eq$ מ@laurent_p_adic_repersention נובע שלכל $x in ZZ_p$, כל סכום חלקי של טור ההצגה ה־$p$־אדית שלו מכיל רק טבעיים ולכן הסדרה היא סדרת מספרים טבעיים ומ@existence_of_p_addic_presentention נובע שהגבול של הסכומים החלקיים האלה הוא $x$.
]
...
#proposition[
  יהיו $s<=t in ZZ$, $d in NN$ ו־$(a_i)_(i=s)^infinity subset.eq ZZ$ כך ש־$0<=a_i <=p-1$ לכל $s<=i$ עם $a_s != 0$ ולכל $t<=i$ מתקיים $a_(i+d) = a_i$.
...
#proposition[מחזוריות הפיתוח ה־$bold(p)$־אדי של מספר רציונלי][
  יהיו $s in ZZ$, $(a_i)_(i=s)^infinity subset.eq ZZ$ כך ש־$0<=a_i <= p-1$ לכל $s<=i$, $a_s != 0$ וגם $sum_(i=s)^infinity a_i p^i in QQ$. אז קיימים $t in ZZ$, $s<=t$ ו־$d in NN$ כך ש־$a_(i+d) = a_i$ לכל $t<=i$.
]

#proof[
  מאחר שהסכום הוא רציונלי, נסמן $sum_(i=s)^infinity a_i p^i = p^s frac(m, n)$ כאשר $p divides.not m, n$, נגדיר $c_i = a_(i+s)$ ונקבל
  $
    frac(m, n) = sum_(i=s)^infinity a_i p^(i-s) = sum_(j=0)^infinity a_(j+s) p^j = sum_(j=0)^infinity c_j p^j ==> m=n sum_(j=0)^infinity c_j p^j
  $
  כעת לכל $k in NN$ נגדיר
  $
    r_k colon.eq frac(n sum_(j=k)^infinity c_j p^j, p^k) = frac(m - n sum_(j=0)^(k-1) c_j p^j, p^k) = n sum_(j=k)^infinity c_j p^(j-k)
  $
  אז $r_k in frac(1, p^k) ZZ_p$ וגם $abs(r_k)_p <=1$ ולכן $r_k in ZZ$. יתר על־כן מכך ש־$0<=c_j <= p-1$ אם ניקח ערך מקסימלי ומינימלי נקבל
  $
    frac(m+n, p^k)-n = frac(m-n(p^k-1), p^k) <= frac(m-n sum_(j=0)^(k-1) (p-1)p^j, p^k) <= frac(m-n sum_(j=0)^(k-1) c_j p^j, p^k) = r_k <= frac(m, p^k)
  $
  משיקולי גבולות קיים $N in NN$ כך שלכל $N<=k$ מתקיים $-n-1<r_k < 1$ ולכן מעיקרון שובך היונים קיימים $N <= k<l$ כך ש־$r_k = r_l$.\
...
#proof[@hensels_lemma][
  ...
  $
    f(r_(k+1)) &= f(r_k + d p^k) \
    &= c_0 + c_1 (r_k + d p^k) + c_2 (r_k + d p^k)^2 + dots.h.c + c_n (r_k + d p^k)^n \
  ...
  נבחין שמכאן נובעת גם היחידות: בכל צעד רקורסיבי הגדרנו את הספרה $d$ להיות יחידה $0<=d<=p-1$ שעבורה התנאי מתקיים ומכיוון שבכל שלב בחירה של הקירוב הבא $r_(k+1)$ היא יחידה לחלוטין, הסדרה הזאת מתכנסת לגבול יחיד ב־$ZZ_p$.
]
