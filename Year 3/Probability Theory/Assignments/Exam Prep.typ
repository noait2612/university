...
#proof[
  נגדיר $B_k = union.big_(k in [n]) A_k$ וזו סדרת מאורעות עולה המקיימת $union.big_(n in NN) B_n = union.big_(n in NN) A_n$, אז
  $
...
#theorem("תיאור משתנה גיאומטרי במונחים של התפלגות שיורית")[
  משתנה מקרי שנתמך על השלמים מתפלג $Geo(p)$ אם ורק אם לכל $n in NN$ מתקיים
  $ PP(X>n) = (1-p)^n $
]
...
#proof[
  ...
  *מפתח להוכחה:* בכיוון הראשון לסדר אינדקס סכימה לטור הנדסי, בכיוון השני להשתמש בהכלת מאורעות כי זה על השלמים.
]
...
#proof[
  ...
  $
    lim_(n arrow.r infinity) PP(abs(X_n-mu)>=epsilon)<=lim_(n arrow.r infinity) PP(abs(X_n - EE(X_n))>=epsilon/2)<=_"אי־שיוויון צ'בישב" lim_(n arrow.r infinity) Var(X_n)/(epsilon^2 4) = 0
  $
  *מפתח להוכחה:*
...
#table(
  ...
  [
    + להגדיר שונות משותפת
    + להוכיח נוסחת סכום שנויות לשני משתנים מקריים
  ],
)
