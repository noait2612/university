...
=== תכונות התוחלת (5.9)
#theorem("תכונות התוחלת")[
  יהיו $X,Y$ משתנים מקריים בעלי תוחלת סופית המוגדרים על אותו מרחב הסתברות. אזי
  + *אי־שליליות:* אם $X >=^(a.s.) 0$ אזי $EE(X)>=0$ ואם בנוסף $PP(X>0)>0$ אזי $EE(X) > 0$
  + *לינאריות:* לכל $a,b in RR$ מתקיים $EE(a X+ b Y)= a EE(X)+b EE(Y)$
  + *מונוטוניות:* אם $X>=^(a.s.) Y$ אז $EE(X)>=EE(Y)$ ואם בנוסף $PP(X>Y)>0$ אזי $EE(X) > EE(Y)$
]

#proof[
  + ישירות מהגדרה
    $
      EE(X) = sum_(x in RR) x PP(X=x) = sum_(x in RR_minus) x PP(X=x) + sum_(x in RR_plus) x PP(X=x) = sum_(x in RR_plus) x PP(X=x) >= 0
    $
  + נגדיר $Z = a X + b Y$ ונניח ש־$EE(abs(a X + b Y))$ קיימת, אזי
  $
    EE(a X + b Y)= EE(Z)=sum_((x,y) in RR^2) (a x + b y) PP(X=x, Y=y) = a sum_(x in RR) (x sum_(y in RR) PP(X=x, Y=y))+b sum_(y in RR) (y sum_(x in RR) PP(X=x, Y=y)) \
    =_"הסתברות שלמה" a sum_(x in RR) x PP(X=x)+b sum_(y in RR) y PP(Y=y)= a EE(X)+b EE(Y)
  $
  אם $X,Y$ אי־שליליים ו־$a, b>=0$ אז הטורים חיוביים ומכך של־$X,Y$ יש תוחלת אז כל הסכום מתכנס ובאופן כללי עם המקרה הפרטי לעיל
  $
    EE(abs(a X + b Y))= sum_((x,y) in RR^2) abs(a x + b y)PP(X=x, Y=y)<=_"אי־שיוויון המשולש" sum_((x,y) in RR^2) (abs(a)abs(x)+abs(b)abs(y)) PP(X=x, Y=y) \
    = EE(abs(a)abs(x)+abs(b)abs(y))=abs(a)EE(abs(X))+abs(b)EE(abs(Y)) < infinity
  $
  + נרשום $X=(X-Y)+Y$ ומהסעיפים הקודמים
    $ EE(X)=EE(X-Y)+EE(Y)>=EE(Y) $
    האי־שיוויון החזק נובע מהסעיף הראשון והקיום של התוחלת של $X-Y$ מהסעיף השני.
]

#pagebreak()

=== אי־שיוויון המשולש של התוחלת (5.10)
#theorem("אי־שיוויון המשולש של התוחלת")[
  לכל שני משתנים מקריים $X,Y$ בעלי תוחלת סופית מתקיים $EE(abs(X+Y))<=EE(abs(X))+EE(abs(Y))$.
]

#proof[
  אי־שיוויון המשולש עבור $a,b in RR$ מקיים
  $ abs(a+b)<=abs(a)+abs(b) $
  ולכן מהגדרה של משתנה מקרי כפונקציה, אז גם לכל $omega$ מתקיים
  $ abs(X(omega)+Y(omega))<=abs(X(omega))+abs(Y(omega)) $
  ובפרט נוכל להסיק
  $ abs(X+Y)<=abs(X)+abs(Y) $
  אז ממונוטוניות ולינאריות התוחלת
  $ EE(abs(X+Y))<=EE(abs(X)+abs(Y))=EE(abs(X))+EE(abs(Y)) $
  הכל מוגדר היטב שכן $X,Y$ בעלי תוחלת סופית ולכן בפרט מתכנס בהחלט.
]
...
#theorem("אי־שיוויון מרקוב – שימוש בתוחלת כיחידה למדידת הסטייה (5.39)")[
  יהי $X$ משתנה מקרי אי־שלילי בעל תוחלת סופית וחיובית.\
  לכל $b>0$ מתקיים
  $ PP(X>=b EE(X))<=1/b $
]

#proof[
  נסמן $a=b EE(X)$ ומכך ש־$b>0, EE(X)> 0$ נובע ש־$a>0$.\
  נסתכל על $bb(1)_({X>=a})$ ומכך ש־$X >=^(a.s.) 0$ נובע ש־$X>=a bb(1)_({X>=a})$ (כי אם $X<a$ אז באגף ימין יש לנו $0$ ו־$X >=^(a.s.) 0$; אם $X>=a$ אז מימין יש לנו $a$ וסיימנו) ומתקיים
  $ EE(X)>=_((1)) EE(a bb(1)_({X>=a}))=_((2)) a PP(X>=a) $
  כאשר $(1)$ זה מונוטוניות התוחלת ו־$(2)$ זה מלינאריות התוחלת ותוחלת של אינדיקטור.\
  כלומר
  $ EE(X)>=b EE(X) PP(X>=b EE(X)) <==> 1 >= b PP(X>= b EE(X)) <==> 1/b >= PP(X>=b EE(X)) $
  כאשר החילוק לא שינה סימן כי הכל חיובי.

  *מפתח להוכחה:*
  + מסמנים $a=b EE(X)$
  + מסבירים למה $X>=a bb(1)_({X>=a})$
  + תכונות התוחלת והצבה לאחור
]
