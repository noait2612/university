...
#subquestion()
נביא דוגמה לפונקציה רציפה ועל $f : ZZ_2 arrow.r I^2$.
#proof[#todo]

#subquestion()
נוכיח כי $ZZ_2$ אינו קשיר מסילתית ונסיק כי לא קיימת פונקציה רציפה ועל $f : I arrow.r ZZ_2$.
#proof[#todo]
...
#subquestion()
נראה כי ניתן לשכן הומיאומורפית את $X$ במרחב המכפלה $I^NN$.
#proof[
  מסעיף א' אנחנו יודעים שיש ${x_1, x_2, ...} subset.eq X$ צפופה.\
  נגדיר $f : X arrow.r I^(NN)$ על־ידי
  $ f(p) = (min{d(p, x_n), 1})_(n=1)^infinity $
  $f$ אכן רציפה כי פונקציית המרחק היא רציפה (וגם ראינו את זה במטלה הקודמת), נשאר להראת ש־$f$ חד־חד ערכית, אבל זה נובע מהצפיפות: אם $p!=q$ אבל $d(p, x_n) = d(q, x_n)$ נובע מהצפיפות כי בהכרח $p=q$ ולכן זו סתירה ו־$f(p)!=f(q)$. #todo
]
